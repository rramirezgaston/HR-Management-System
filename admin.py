...
class AdminApp(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.withdraw()
        self.title("HR System Administration")
    ...
    def create_jobs_tab(self, parent_frame):
        ...
        self.jobs_tree = ttk.Treeview(display_frame, columns=cols, show='headings')
        for col in cols:
            self.jobs_tree.heading(col, text=col.replace('_', ' ').title())
        v_scroll = ttk.Scrollbar(display_frame, orient=tk.VERTICAL, command=self.jobs_tree.yview)
        self.jobs_tree.configure(yscroll=v_scroll.set)
        h_scroll = ttk.Scrollbar(display_frame, orient=tk.HORIZONTAL, command=self.jobs_tree.xview)
        self.jobs_tree.configure(xscroll=h_scroll.set)
    ...
    def load_jobs(self):
        ...
        try:
            ...
            for row in cursor.fetchall():
                self.jobs_tree.insert('', tk.END, iid=row[0], values=row[1:])
            conn.close()
    ...
    def open_edit_job_window(self, event=None):
        ...
        if not selection: 
            if event is None:
                messagebox.showwarning("Selection Error", "Please select a job to edit.", parent=self)
            return
    ...
    def create_interviewers_tab(self, parent_frame):
        ...
        self.interviewers_listbox.pack(fill=tk.BOTH, expand=True)
    ...
    def load_interviewers(self):
        ...
        try:
            ...
            for id, name in cursor.fetchall():
                self.interviewers_listbox.insert(tk.END, name)
                self.interviewers_map[name] = id
            conn.close()
        except sqlite3.Error as e:
    ...
    def open_edit_interviewer_window(self, event=None):
        ...
        if not selection: 
            if event is None:
                messagebox.showwarning("Selection Error", "Please select an interviewer to edit.", parent=self)
            return
        name = self.interviewers_listbox.get(selection[0])
        interviewer_id = self.interviewers_map.get(name)
    ...
    def delete_interviewer(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM Candidate_Interviewers WHERE fk_interviewer_id = ?", (interviewer_id,))
        count = cursor.fetchone()[0]
        conn.close()
        ...
        if messagebox.askyesno("Confirm Deletion", msg, icon='warning', parent=self):
            try:
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM Interviewers WHERE interviewer_id = ?", (interviewer_id,))
                conn.commit()
                conn.close()
                self.refresh_all_tabs()
            except sqlite3.Error as e:
    ...
    def load_classes(self):
        ...
        try:
            ...
            for id, date_str in cursor.fetchall():
                self.classes_listbox.insert(tk.END, date_str)
                self.classes_map[date_str] = id
            conn.close()
        except sqlite3.Error as e:
    ...
    def open_edit_class_window(self, event=None):
        ...
        if not selection: 
            if event is None:
                messagebox.showwarning("Selection Error", "Please select a class to edit.", parent=self)
            return
        date_str = self.classes_listbox.get(selection[0])
        class_id = self.classes_map.get(date_str)
    ...
    def delete_class(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM Candidates WHERE fk_class_id = ?", (class_id,))
        count = cursor.fetchone()[0]
        conn.close()
        ...
        if messagebox.askyesno("Confirm Deletion", msg, icon='warning', parent=self):
            try:
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM Hiring_Classes WHERE class_id = ?", (class_id,))
                conn.commit()
                conn.close()
                self.refresh_all_tabs()
            except sqlite3.Error as e:
...
class AddJobWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        ttk.Label(main_frame, text="Shift:").pack(anchor=tk.W)
        self.shift_entry = ttk.Entry(main_frame, width=40)
        self.shift_entry.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(main_frame, text="Pay Structure:").pack(anchor=tk.W)
        self.pay_combo = ttk.Combobox(main_frame, values=['Hourly', 'Salary'], state="readonly")
        self.pay_combo.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(main_frame, text="Employment Type:").pack(anchor=tk.W)
        self.type_combo = ttk.Combobox(main_frame, values=['Full-Time', 'Part-Time'], state="readonly")
    ...
    def add_job(self):
        ...
        emp_type = self.type_combo.get()
        if not all([dept, pay, emp_type]):
            messagebox.showwarning("Input Error", "Department, Pay Structure, and Employment Type are required.", parent=self)
            return
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("INSERT INTO Jobs (department, shift, pay_structure, employment_type) VALUES (?, ?, ?, ?)", (dept, shift or None, pay, emp_type))
            conn.commit()
            conn.close()
            messagebox.showinfo("Success", "Job added successfully.", parent=self)
...
class EditJobWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        ttk.Label(main_frame, text="Department:").pack(anchor=tk.W)
        ttk.Entry(main_frame, textvariable=self.dept_var, width=40).pack(fill=tk.X, pady=(0, 10))
        ttk.Label(main_frame, text="Shift:").pack(anchor=tk.W)
        ttk.Entry(main_frame, textvariable=self.shift_var, width=40).pack(fill=tk.X, pady=(0, 10))
        ttk.Label(main_frame, text="Pay Structure:").pack(anchor=tk.W)
        ttk.Combobox(main_frame, textvariable=self.pay_var, values=['Hourly', 'Salary'], state="readonly").pack(fill=tk.X, pady=(0, 10))
        ttk.Label(main_frame, text="Employment Type:").pack(anchor=tk.W)
        ttk.Combobox(main_frame, textvariable=self.type_var, values=['Full-Time', 'Part-Time'], state="readonly").pack(fill=tk.X, pady=(0, 10))
    ...
    def load_job_data(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM Jobs WHERE job_id = ?", (self.job_id,))
            data = cursor.fetchone()
            conn.close()
            if data:
                self.dept_var.set(data['department'])
                self.shift_var.set(data['shift'] or '')
                self.pay_var.set(data['pay_structure'])
                self.type_var.set(data['employment_type'])
        except sqlite3.Error as e:
            messagebox.showerror("Load Error", f"Could not load job data: {e}", parent=self)
            self.destroy()
    ...
    def save_changes(self):
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE Jobs SET department = ?, shift = ?, pay_structure = ?, employment_type = ? WHERE job_id = ?", (dept, shift or None, pay, emp_type, self.job_id))
            conn.commit()
            conn.close()
            messagebox.showinfo("Success", "Job details updated successfully.", parent=self)
            self.parent.refresh_all_tabs()
            self.destroy()
        except sqlite3.Error as e:
...
class AddEditInterviewerWindow(tk.Toplevel):
    def __init__(self, parent, interviewer_id=None, interviewer_name=""):
        ...
        self.title("Edit Interviewer" if interviewer_id else "Add New Interviewer")
        ...
        main_frame = ttk.Frame(self, padding="20")
        main_frame.pack(fill=tk.BOTH, expand=True)
        ttk.Label(main_frame, text="Interviewer Name:").pack(anchor=tk.W)
        ttk.Entry(main_frame, textvariable=self.name_var, width=40).pack(fill=tk.X, pady=(0, 10))
        ttk.Button(main_frame, text="Save Changes", command=self.save).pack()
        center_window(self, parent)
    ...
    def save(self):
        ...
        try:
            ...
            if self.interviewer_id:
                cursor.execute("UPDATE Interviewers SET interviewer_name = ? WHERE interviewer_id = ?", (name, self.interviewer_id))
            else:
                cursor.execute("INSERT INTO Interviewers (interviewer_name) VALUES (?)", (name,))
            conn.commit()
            conn.close()
            self.parent.refresh_all_tabs()
            self.destroy()
        except sqlite3.IntegrityError:
            messagebox.showerror("Input Error", f"An interviewer named '{name}' already exists.", parent=self)
        except sqlite3.Error as e:
...
class AddEditClassWindow(tk.Toplevel):
    def __init__(self, parent, class_id=None, class_date=""):
        ...
        self.title("Edit Class Date" if class_id else "Add New Class")
        self.geometry("400x150")
        self.transient(parent)
        self.grab_set()
        ...
        main_frame = ttk.Frame(self, padding="20")
        main_frame.pack(fill=tk.BOTH, expand=True)
        ttk.Label(main_frame, text="Class Date (YYYY-MM-DD):").pack(anchor=tk.W)
        ttk.Entry(main_frame, textvariable=self.date_var, width=40).pack(fill=tk.X, pady=(0, 10))
        ttk.Button(main_frame, text="Save Changes", command=self.save).pack()
        center_window(self, parent)
    ...
    def save(self):
        ...
        try:
            ...
            messagebox.showwarning("Input Error", "Invalid date format. Please use YYYY-MM-DD.", parent=self)
            return
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            if self.class_id:
                cursor.execute("UPDATE Hiring_Classes SET class_date = ? WHERE class_id = ?", (date_str, self.class_id))
            else:
                cursor.execute("INSERT INTO Hiring_Classes (class_date) VALUES (?)", (date_str,))
            conn.commit()
            conn.close()
            self.parent.refresh_all_tabs()
            self.destroy()
        except sqlite3.IntegrityError:
            messagebox.showerror("Input Error", f"The class date '{date_str}' already exists.", parent=self)
        except sqlite3.Error as e:
            messagebox.showerror("DB Error", f"Failed to save class date: {e}", parent=self)
